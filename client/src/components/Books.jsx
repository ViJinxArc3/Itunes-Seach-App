...
const Books = () => {
 ...
 // searchBook, searchResults, and searchClicked.
...
// call addToFavorites function with the liked item data when the like 
// button is clicked
...
// clicked and updates the searchResults and search Clicked state variables
  const handleSearchClick = async () => {
    const response = await fetch(`/search?term=${searchBook}&entity=ebook`);
  ...
  return (
    <div>
        ...
        <h1>iBooks</h1>
        <Link to='/'>
        <button className='home-btn-books'>Homepage</button>
        </Link>
        </div>
        <div className='header'>
            <h2>Read, listen, discover. All in one place.</h2>
        
        </div>
        <div className='header'>
            <p>Apple Books is the single destination 
            for all the books and audiobooks you love — 
            and all the ones you will love next. Browse the 
            Book Store and Audiobook Store to find the 
            perfect book to read or listen to. Track what
             you have read and want to read, and set your 
             own Reading Goals — all in one app and across 
             all your Apple devices.</p>
             </div>
      ...
      for each item and displays name of artist, book name and image */}
